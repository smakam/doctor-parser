...
from app.schemas.nameboard import (
    ExtractionResponse, ExtractedData, ExtractedField,
    ImageQualityResult, PiiData,
)
...
def _build_extracted_data(extraction: ExtractionResult, geocoding: GeocodingResult) -> ExtractedData:
...
def _build_image_quality(
    ...
    for ocr in ocr_results:
        warnings = []
        if ocr.quality == "POOR":
            warnings.append("Low OCR confidence — check this image for obstructions or blur.")
        if not ocr.raw_text.strip():
            warnings.append("No text detected in this image.")
        output.append(ImageQualityResult(
...
def _build_warnings(ocr_results: list[ImageOcrResult], extraction: ExtractionResult) -> list[str]:
    warnings = []
    poor_images = sum(1 for o in ocr_results if o.quality == "POOR")
    if poor_images:
        warnings.append(f"{poor_images} image(s) had low OCR confidence. Review highlighted fields carefully.")
    if not extraction.doctor_name or not extraction.doctor_name.value:
        warnings.append("Doctor name could not be extracted — please enter it manually.")
